 ...
 * StatsdClient.java
 *
 * (C) 2011 Meetup, Inc. Author: Andrew Gwozdziewycz <andrew@meetup.com>,
 * @apgwoz
 *
 *
 *
 ...
 * increment by 1 client.increment("foo.bar.baz"); // increment by 10
 * client.increment("foo.bar.baz", 10); // sample rate
 * client.increment("foo.bar.baz", 10, .1); // increment multiple keys by 1
 * client.increment("foo.bar.baz", "foo.bar.boo", "foo.baz.bar"); // increment
 * multiple keys by 10 -- yeah, it's "backwards" client.increment(10,
 * "foo.bar.baz", "foo.bar.boo", "foo.baz.bar"); // multiple keys with a sample
 * rate client.increment(10, .1, "foo.bar.baz", "foo.bar.boo", "foo.baz.bar");
 *
 ...
 *
 * @license
 * https://github.com/etsy/statsd/blob/576f977e30cbac6475e4615223422661c3ddc486/LICENSE
 */
